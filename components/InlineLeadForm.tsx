...
export const InlineLeadForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackEvent('lead_submitted', { source: 'inline_form', country: 'US' });
      setSubmitted(true);
    } catch (err) {
  ...
  if (submitted) {
    return (
      <div className="bg-primary-light rounded-2xl p-8 md:p-10 text-center">
        ...
        <h3 className="text-2xl font-bold text-neutral-900 mb-3">Thanks for your request!</h3>
        <p className="text-neutral-500">We'll contact you within 24 hours with matching quotes.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface rounded-2xl p-8 md:p-10 border border-neutral-300/30">
      <h3 className="text-2xl font-bold text-neutral-900 mb-2">Get free quotes</h3>
      <p className="text-neutral-500 text-sm mb-6">Enter your details and receive no-obligation quotes within 24 hours.</p>
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {submitting ? <Loader2 size={18} className="animate-spin" /> : <>Get Free Quotes <ArrowRight size={16} /></>}
        </button>
      </form>
    </div>
  );
