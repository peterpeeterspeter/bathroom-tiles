...
4.  **Processing**:
    ...
    -   `generateRenovation()` creates a single-shot render. Bathroom photo is always IMAGE 1 (first in parts array), before inspiration and product images. Prompt opens with "IMAGE 1 IS YOUR GROUND TRUTH" and all references use "IMAGE 1" consistently. Structure: ground-truth declaration → STAGE 1 (study room, perspective lock + natural room description) → STAGE 2 (mentally strip room to bare shell) → STAGE 3 (place fixtures one by one from empty room) → STAGE 4 (apply finishes and style) → STAGE 5 (final verification). Uses Gemini 3's built-in thinking/reasoning (default high) for step-by-step spatial planning. The natural description (from analysis) replaces the machine-assembled buildRoomDescription() for more accurate room locking.
